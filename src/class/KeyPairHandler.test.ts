...
import { KeyPairHandler } from "./KeyPairHandler";
...
describe("KeyPairHandler.ts", () => {
  let crypto: KeyPairHandler;
  ...
  describe("RS512", () => {
    ...
    test("should sign/assert", () => {
      expect(crypto.assert("string", signature)).toBe(undefined);
    }, 10000);

    test("should sign/throw error", () => {
      expect(() => crypto.assert("wrong", signature)).toThrow(expect.any(KeyPairAssertError));
    }, 10000);
